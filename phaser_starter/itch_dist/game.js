...
// Audio logic moved to AudioManager.js
...
function create() {
    ...
    // Initialize HUD Manager (Stats, Ability Bar, Potion Slots)
    if (window.HUDManager) {
        window.HUDManager.init(this);
        window.HUDManager.createHUD();
    } else {
        console.error('HUDManager not found!');
    }
...
function resize(gameSize) {
    ...
    // --- 3. Update Ability Bar & HUD ---
    if (window.HUDManager) {
        window.HUDManager.resize(width, height);
    }
}
